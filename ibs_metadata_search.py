...
# -*- coding: utf-8 -*-
#
# ibs_metadata_search.py
# EPUB Metadata Scraper & Embedder
#
# Searches book metadata from ibs.it and embeds it into EPUB files.
# Designed for Italian language books.
#
# Usage:
#   python ibs_metadata_search.py <directory> [--force] [--json-only]
#
# Requirements:
...
try:
    ...
    print('Warning: playwright library not found. Install with: pip install playwright')
    print('The search functionality requires Playwright for JavaScript rendering.')


# Configuration
BASE_URL = 'https://www.ibs.it'
SEARCH_URL = BASE_URL + '/algolia-search?ts=as&query='
...
def _init_browser():
    '''Initialize Playwright browser (lazy init). Uses system Chrome via DevTools protocol.'''
...
def _get_cached_page():
    '''Get or create a cached Playwright page for reuse across search calls.
    Returns (page, is_new) tuple where is_new indicates if a fresh context/page was created.'''
    global _playwright_context, _playwright_page, _algolia_listener_attached
    browser = _init_browser()
    if browser is None:
        return None, False
    try:
        # If cached page and context exist and are still usable, return them
        if _playwright_page is not None and _playwright_context is not None:
            # Quick health check - try to evaluate something simple
            try:
                _playwright_page.evaluate('1 + 1')
                return _playwright_page, False
            except Exception:
                pass
        ...
        try:
            ...
            pass
        # Reset listener flag so it will be attached to the new page
        _algolia_listener_attached = False
        _playwright_context = browser.new_context(
            user_agent=HEADERS['User-Agent'],
            locale='it-IT',
        )
        _playwright_page = _playwright_context.new_page()
        return _playwright_page, True
    except Exception:
        _playwright_context = None
        _playwright_page = None
        return None, False
...
def _close_browser():
    '''Close Playwright browser, cached page/context, and stop the Playwright instance.'''
    global _playwright, _browser, _playwright_context, _playwright_page
    try:
        if _playwright_page:
...
# IBS.it Search Functions

# Global cache for Algolia XHR interception
_algolia_responses = []
_algolia_listener_attached = False


def _on_algolia_response(response):
    '''Persistent handler for intercepting Algolia API responses via Playwright.
    Stores structured data in the global _algolia_responses list.'''
    if 'algolia.net' in response.url and '/queries' in response.url:
        try:
            data = response.json()
            _algolia_responses.append(data)
        except Exception:
            pass


def _attach_algolia_listener(page):
    '''Attach the persistent Algolia response listener to a page if not already done.'''
    global _algolia_listener_attached
    if not _algolia_listener_attached:
        page.on('response', _on_algolia_response)
        _algolia_listener_attached = True


def _parse_algolia_hits(algolia_responses):
    '''Parse raw Algolia API responses into structured book metadata dicts.
    Deduplicates by URL and returns a list of results.'''
    results = []
    seen_urls = set()
    for data in algolia_responses:
...
def search_ibs(query):
    '''Search for books on ibs.it via Algolia search.
    Uses Playwright to intercept the Algolia API response which contains
    structured metadata (title, author, publisher, ISBN, year, etc.).
    Reuses a cached Playwright page across calls for batch performance.
    Returns list of dicts with complete metadata extracted from the JSON.
    '''
    global _algolia_responses

    page, is_new = _get_cached_page()
    if page is None:
        return []

    url = SEARCH_URL + quote(query)

    # Attach persistent listener if creating a fresh page
    if is_new:
        _attach_algolia_listener(page)

    # Clear previous responses before this navigation
    _algolia_responses.clear()

    try:
        # Navigate and wait for the Algolia response to arrive
        page.goto(url, wait_until='networkidle', timeout=30000)

        # Brief pause to ensure response callback processes
        time.sleep(0.5)

        # Parse collected Algolia hits into structured results
        return _parse_algolia_hits(_algolia_responses)

    except Exception:
        return []
    # NOTE: Context is NOT closed here - it's cached and reused.
    # Cleanup happens via _close_browser() registered with atexit.
...
def find_book_metadata(author, title):
    ...
    Uses Algolia search results (rich metadata) and fetches detail page only for description.
    '''
    ...
    # Try combined author+title query (only one attempt per query - Playwright is slower)
    # But we still need to manage the Playwright context efficiently
    for q in [query, title, clean_author]:
        r = search_ibs(q)
        if r:
            results = r
            break
...
<head>
    ...
    <title>EPUB Metadata Pipeline (IBS.it) - Report</title>
    ''' + css + '''
</head>
<body>
    <div class="container">
        <header>
            <h1>EPUB Metadata Pipeline &mdash; IBS.it</h1>
...
# Metadata Report Generator (previously in epub_metadata_report.py)
...
def generate_metadata_report(books, output_path):
    ...
    for i, book in enumerate(books, 1):
        ...
        fields_html = ''
        if identifier:
            fields_html += _metadata_field('ISBN', identifier, '📖')
        else:
            fields_html += _metadata_field('ISBN', '\u2014', '📖')

        if publisher:
            fields_html += _metadata_field('Editore', publisher, '🏢')
        else:
            fields_html += _metadata_field('Editore', '\u2014', '🏢')

        if language:
            lang_name = {'it': 'Italiano', 'en': 'Inglese', 'fr': 'Francese',
                         'de': 'Tedesco', 'es': 'Spagnolo', 'pt': 'Portoghese'}.get(language.lower(), language)
            fields_html += _metadata_field('Lingua', lang_name, '🌐')
        else:
            fields_html += _metadata_field('Lingua', '\u2014', '🌐')
...
def run_pipeline(wdir, delay=2.0, verbose=False, force=False, json_only=False):
    '''Run the complete metadata pipeline. Returns exit code (0 for success).'''
...
def main():
    parser = argparse.ArgumentParser(
        description='EPUB Metadata Pipeline (IBS.it) - Search and embed metadata from ibs.it',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  python ibs_metadata_search.py ./books              # Interactive mode
  python ibs_metadata_search.py ./books --force      # Auto-embed after scraping
  python ibs_metadata_search.py ./books --json-only  # Only scrape, no embed
  python ibs_metadata_search.py ./books --delay 3.0  # Use 3s delay between requests
        '''
    )
